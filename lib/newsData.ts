...
function detectCategory(title: string, description: string): { category: string; categoryStyle: string } {
  ...
  const results = await Promise.allSettled(
    FEEDS.map(async (feed) => {
      ...
      return data.items.map((item, i): NewsItem => {
        const { category, categoryStyle } = detectCategory(item.title, item.description);
