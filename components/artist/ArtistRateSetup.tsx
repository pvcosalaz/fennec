 ...
 * show. El resultado se recalcula en vivo abajo, asi que ver todo junto ES la
 * herramienta — mueves un numero y ves la tarifa moverse.
 *
 * Dos presentaciones del mismo formulario:
 *  - overlay (telefono): hoja que sube desde abajo, como las demas hojas.
 *  - inline (escritorio): VISTA que sustituye al hub, como la calculadora del
 *    productor. Un formulario de veinte campos metido en un pop-up se siente
 *    de telefono en una pantalla grande (Paco 2026-08-19).
 *
...
function Seccion({ title, cols = 2, children }: { title: string; cols?: number; children: React.ReactNode }) {
  return (
    <section className="space-y-3">
      ...
      <div className={cols === 3 ? "grid grid-cols-2 gap-3 lg:grid-cols-3" : "grid grid-cols-2 gap-3"}>{children}</div>
    </section>
  );
}

export default function ArtistRateSetup({
  onClose, onSaved, inline = false,
}: {
  onClose: () => void;
  onSaved: (s: ArtistPricingState) => void;
  /** true = vista de escritorio que sustituye al hub; false = hoja overlay. */
  inline?: boolean;
}) {
  ...
  const secciones = (
    <>
      <Seccion title={t("abSecLife")} cols={inline ? 3 : 2}>
      ...
      <Seccion title={t("abSecProject")} cols={inline ? 3 : 2}>
    ...
    return (
      <div className="mx-auto w-full max-w-3xl">
        <div className="mb-6 flex items-center gap-3">
          <button onClick={onClose} aria-label={t("abCancel")} className="text-zinc-400 transition hover:text-accent">
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div>
            <h1 className="text-[21px] font-bold tracking-tight text-white">{t("abSetupTitle")}</h1>
            <p className="mt-0.5 max-w-[60ch] text-[12px] leading-snug text-zinc-500">{t("abSetupIntro")}</p>
          </div>
        </div>

        <div className="space-y-8">{secciones}</div>

        <div className="mt-8 rounded-2xl border border-accent/20 bg-accent/[0.04] px-6 py-5">
          {resultado}
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-white/10 bg-[#131216] sm:rounded-3xl">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">{secciones}</div>

        <div className="border-t border-white/[0.06] bg-white/[0.02] px-6 py-4">{resultado}</div>
      </div>
    </div>
  );
}
