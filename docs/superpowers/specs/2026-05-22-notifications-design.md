...
Add an in-app notification feed and Web Push notifications to Fennec. Users receive timely, AI-generated alerts for audio feedback, upcoming content publish dates, and project deadlines.
...
2. **Bell icon in the app header** — left side, same row as the settings gear, shows unread badge, opens a slide-down feed
3. **Web Push (VAPID)** — service worker + push subscription stored in Supabase, serverless cron sends pushes daily and on feedback events
...
| type | text | `audio_feedback` \| `content_scheduled` \| `project_deadline` |
| title | text | AI-generated short message (max 80 chars) |
| body | text | Optional secondary line (e.g. track title, project name) |
...
Logic:
1. Fetch all projects where `deadline = tomorrow` → create `project_deadline` notification + push per owner
2. Fetch all content tasks where `date = today` → create `content_scheduled` notification + push per owner
...
- Per-type notification preferences (on/off toggles) — deferred
